...
let textToImg = (emoText) => {
    ...
    if (idx >= 0) {
        ...
        return `<img src="${url}" style="width:35px;height:35px;vertical-align:bottom;" data-code="#${emoTextList[idx]};"/>`
    }

    idx = originalEmoTextList.indexOf(word)
    if (idx >= 0) {
        let url = require(`@/assets/emoji/emo1/${idx}.gif`);
        return `<img src="${url}" style="vertical-align:bottom;" data-code="#${originalEmoTextList[idx]};"/>`
    }
    if (idx === -1) {
        return emoText;
    }
