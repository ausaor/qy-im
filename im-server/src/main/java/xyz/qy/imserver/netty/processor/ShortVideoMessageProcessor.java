...
public class ShortVideoMessageProcessor extends AbstractMessageProcessor<IMRecvInfo> {

    @Override
    public void process(IMRecvInfo recvInfo) {
        log.info("接收到短视频消息,接收用户数量:{}，内容:{}",  recvInfo.getReceivers().size(), recvInfo.getData());
        for (IMUserInfo receiver : recvInfo.getReceivers()) {
            try {
                ...
                if (!Objects.isNull(channelCtx)) {
                    ...
                    sendInfo.setCmd(IMCmdType.SHORT_VIDEO_MESSAGE.code());
