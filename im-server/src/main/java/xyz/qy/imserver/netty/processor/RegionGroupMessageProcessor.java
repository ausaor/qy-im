...
@Slf4j
@Component
public class RegionGroupMessageProcessor extends AbstractMessageProcessor<IMRecvInfo> {
    ...
    public void process(IMRecvInfo recvInfo) {
        ...
        log.info("接收到地区群聊消息，发送者:{},接收用户数量:{}，内容:{}", sender.getId(), receivers.size(), recvInfo.getData());
        for (IMUserInfo receiver : receivers) {
            try {
                ...
                if (channelCtx != null) {
                    ...
                    sendInfo.setCmd(IMCmdType.REGION_GROUP_MESSAGE.code());
    ...
    private void sendResult(IMRecvInfo recvInfo, IMUserInfo receiver, IMSendCode sendCode) {
        if (recvInfo.getSendResult()) {
            ...
            // 推送到结果队列
            String key = StrUtil.join(":", IMRedisKey.IM_RESULT_REGION_GROUP_QUEUE, recvInfo.getServiceName());
            redisTemplate.opsForList().rightPush(key, result);
            redisTemplate.convertAndSend(IMConstant.REGION_GROUP_MSG_SEND_RESULT_TOPIC, key);
        }
    }
}
